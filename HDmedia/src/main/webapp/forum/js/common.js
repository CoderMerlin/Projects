...
function showMenu(v) {
	...
	if(v.noskip){
		jQuery("#"+menuObj.id).find('.altw').css({'width':v.dia_width,'max-height':'640px','overflow':'auto'});
	}
	if(pos != '*') {
...
function dragMenu(menuObj, e, op) {
	jQuery&&jQuery('.ui-autocomplete').hide();
	e = e ? e : window.event;
	if(op == 1) {
...
function setMenuPosition(showid, menuid, pos,is_nav) {
	...
	if(direction == 0 || menuObj.scrolly) {
		if(BROWSER.ie && BROWSER.ie < 7) {
			...
			menuObj.style.position = 'fixed';//mobileplayer() ? 'absolute' : 'fixed';
		}
	}
	//如果是导航 则left减去40 居中
	//if(is_nav) ml = ml - 30;
...
//拓展参数pos、dia_width
function showDialog(msg, mode, t, func, cover, funccancel, leftmsg, confirmtxt, canceltxt, closetime, locationtime, pos, dia_width, noskip) {
	if( discuz_uid == 0 && mode == 'info' && isUndefined(func) && isUndefined(funccancel) && isUndefined(leftmsg) && isUndefined(confirmtxt)){
	...
	showMenu({'mtype':'dialog','menuid':menuid,'duration':3,'pos': pos || '00','zindex':JSMENU['zIndex']['dialog'],'cache':0,'cover':cover, dia_width:dia_width, noskip: noskip});
	try {
...
function simulateSelect(selectId, widthvalue) {
	...
	$(selectId + '_ctrl').onclick = function(e) {
		...
		if(selectId=='phone_brands'||selectId=='phone_device'){
			$(selectId + '_ctrl_menu').style.width = 227+'px';
		}
...
function mobileplayer() {
	...
	var andriod = ua.indexOf( "Android" ) > -1 || ua.indexOf('Mobile') > -1;
	if(ios || andriod) {
		return true;
	} else {
		return false;
...
function checkImage(url){
    ...
    if(url.indexOf('.')>-1){
        urls = url.split('.');
        if(typeof urls[1] != "undefined" && (urls[1]=='png'||urls[1]=='jpg'||urls[1]=='gif'||urls[1]=='bmp')){
            return true;
        }
        return false;
    }else {
